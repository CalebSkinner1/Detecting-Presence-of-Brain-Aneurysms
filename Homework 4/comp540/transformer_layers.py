...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        i = torch.arange(max_len, dtype=torch.float32).unsqueeze(1) # shape (max_len, 1)
        div_term = 10000.0 ** (-torch.arange(0, embed_dim, 2, dtype=torch.float32) / embed_dim) # shape (embed_dim/2,)
    ...
    def forward(self, x):
        ...
        ############################################################################
        pos_enc = self.pe[:, :S, :] # shape (1, S, D) (x's sequence length may NOT be max_length so need to slice first S rows)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        ############################################################################
        H, Hd = self.n_head, self.head_dim # Hd = E/H
        ...
        # split into heads
        Q = Q.reshape(N, S, H, Hd).transpose(1, 2) # (N, H, S, Hd)
        K = K.reshape(N, T, H, Hd).transpose(1, 2) # (N, H, T, Hd)
        V = V.reshape(N, T, H, Hd).transpose(1, 2) # (N, H, T, Hd)

        scores = torch.matmul(Q, K.transpose(-2, -1)) / self.scale # (N, H, S, T)
        
        if attn_mask is not None:
            mask = (attn_mask == 0).unsqueeze(0).unsqueeze(0) # (1,1,S,T)
            scores = scores.masked_fill(mask, float('-inf'))
            
        attn = torch.softmax(scores, dim=-1) # (N, H, S, T)
        attn = self.attn_drop(attn)
        
        Y = torch.matmul(attn, V) # (N, H, S, Hd)
